...
const 팝업창 = ({ 
    ...
    const 개별상품이미지추출 = async (상품) => {
        ...
        try {
            ...
            console.log('🐥🐥🐥🐥🐥 이미지 추출 API 응답:', {
                success: data.success,
                결과수: data.결과?.length || 0
            });
            
            if (data.success && data.결과 && data.결과.length > 0) {
                const 결과 = data.결과[0];
                console.log('🐥🐥🐥🐥🐥 개별 상품 결과:', {
                    성공: 결과.성공,
                    이미지수: 결과.이미지들?.length || 0,
                    상세이미지수: 결과.상세이미지들?.length || 0
                });
                
                if (결과.성공 && (결과.이미지들 || 결과.상세이미지들)) {
                    console.log('🐥🐥🐥🐥🐥 추출된 썸네일 이미지들:', 결과.이미지들?.length || 0, '개');
                    console.log('🐥🐥🐥🐥🐥 추출된 상세 이미지들:', 결과.상세이미지들?.length || 0, '개');
                    ...
                    console.log('🐥🐥🐥🐥🐥 필터링된 썸네일 이미지들:', 필터된썸네일이미지들.length, '개');
                    console.log('🐥🐥🐥🐥🐥 필터링된 상세 이미지들:', 필터된상세이미지들.length, '개');
                    
                    // 🐥🐥🐥🐥🐥 앞에서부터 5개만 사용
                    const 최종썸네일이미지들 = 필터된썸네일이미지들.slice(0, 5);
                    const 최종상세이미지들 = 필터된상세이미지들.slice(0, 5);
                    
                    console.log('🐥🐥🐥🐥🐥 최종 썸네일 이미지들:', 최종썸네일이미지들.length, '개');
                    console.log('🐥🐥🐥🐥🐥 최종 상세 이미지들:', 최종상세이미지들.length, '개');
                    ...
                    console.log('🐥🐥🐥🐥🐥 변환된 상품:', {
                        ...
                        카테고리: 변환된상품.카테고리,
                        가격: 변환된상품.가격,
                        썸네일이미지수: 변환된상품.썸네일이미지들?.length || 0,
                        상세이미지수: 변환된상품.상세이미지들?.length || 0
                    });
                    return 변환된상품;
                } else {
                    console.log('🐥🐥🐥🐥🐥 이미지 추출 실패 또는 이미지 없음:', {
                        성공: 결과.성공,
                        이미지수: 결과.이미지들?.length || 0,
                        상세이미지수: 결과.상세이미지들?.length || 0
                    });
